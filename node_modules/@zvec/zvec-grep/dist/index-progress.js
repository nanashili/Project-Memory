...
export function formatIndexProgressLine(progress) {
...
export function formatModelDownloadProgress(progress) {
    ...
    if (embedding.stage === "warning") {
        return `zvec-grep warning: ${singleLineText(embedding.message ?? "Embedding model warning")}`;
    }
    ...
    if (typeof downloadedBytes === "number" &&
        ...
        totalBytes > 0) {
        const percent = Math.min(100, Math.max(0, Math.round((downloadedBytes / totalBytes) * 100)));
        parts.push(`${percent}%`);
        parts.push(`${formatByteCount(downloadedBytes)}/${formatByteCount(totalBytes)}`);
    }
...
export function indexProgressMessage(progress) {
    const line = formatIndexProgressLine(progress);
    return line
        ? `${MCP_INDEX_PROGRESS_PREFIX}${JSON.stringify({ line, progress })}`
        : undefined;
}
export function indexProgressFromMessage(message) {
    if (!message?.startsWith(MCP_INDEX_PROGRESS_PREFIX))
        return undefined;
    const payload = message.slice(MCP_INDEX_PROGRESS_PREFIX.length);
    if (payload.startsWith("{")) {
        try {
            const decoded = JSON.parse(payload);
            if (typeof decoded.line === "string" &&
...
function isIndexProgress(value) {
    if (!value || typeof value !== "object")
        return false;
    const progress = value;
    return (progress.phase === "scanning" ||
        progress.phase === "indexing" ||
        progress.phase === "done");
}
function formatFailedProgress(progress) {
    const failed = progress.filesFailed ?? 0;
    return failed > 0 ? ` [failed=${failed}]` : "";
}
function formatEmbeddingProgress(progress) {
    const embedding = progress.embedding;
    if (!embedding) {
        return "";
    }
    ...
    if (typeof embedding.retryableFailures === "number" &&
...
//# sourceMappingURL=index-progress.js.map