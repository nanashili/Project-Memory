...
import { embedModel2VecTexts, sharedStaticEmbeddingTable, } from "./model2vec-runtime.js";
...
const defaultDependencies = {
    ...
    async download(url, destination, onProgress) {
        ...
        const parsedTotalBytes = contentLength
            ...
            : undefined;
        const totalBytes = parsedTotalBytes !== undefined &&
        ...
        await pipeline(Readable.fromWeb(response.body), progressStream, createWriteStream(destination));
    },
};
export class Model2VecEmbeddingModel extends BaseEmbeddingModel {
    ...
    async loadModel(onProgress) {
        const downloadProgress = createModelDownloadProgressReporter(this.entry.reference, onProgress, [basename(this.entry.modelFile), basename(this.entry.tokenizerFile)]);
        ...
        const staticTable = await this.dependencies.loadSafetensors(modelPath, this.entry.embeddingTensor, this.info.dimension);
        this.ensureNotDisposed();
        if (this.useWorkerPool) {
            ...
            const workerPool = new Model2VecWorkerPool({
                ...
                tableBuffer: sharedTable.data.buffer,
    ...
    async resolveModelPath(downloadProgress) {
        const modelPath = join(this.modelCacheDir, "model2vec", this.entry.repo.replaceAll("/", "--"), this.entry.revision, basename(this.entry.modelFile));
        return await this.resolveCachedFile(this.entry.modelFile, modelPath, downloadProgress);
    }
    async resolveTokenizerSource(downloadProgress) {
        const tokenizerDirectory = join(this.modelCacheDir, "model2vec", this.entry.repo.replaceAll("/", "--"), this.entry.revision, "tokenizer");
        await this.resolveCachedFile(this.entry.tokenizerFile, join(tokenizerDirectory, "tokenizer.json"), downloadProgress);
        const configPath = join(tokenizerDirectory, "tokenizer_config.json");
        if (!isUsableModelFile(configPath)) {
            await writeFile(configPath, `${JSON.stringify({ tokenizer_class: "PreTrainedTokenizer" })}\n`);
        }
        return tokenizerDirectory;
    }
    async resolveCachedFile(remoteFile, localPath, downloadProgress) {
    ...
    async embedTexts(texts) {
        ...
        return await embedModel2VecTexts(texts, tokenizer, staticTable, this.entry.maxInputTokens, this.entry.normalize);
    }
    ensureNotDisposed() {
        if (this.disposed) {
...
function isUsableModelFile(path) {
...
async function readStaticEmbeddingTable(path, tensorName, expectedDimension) {
    ...
    try {
        ...
        if (!tensor ||
            ...
            throw new Error(`Safetensors tensor '${tensorName}' is missing or incompatible`);
        }
        ...
        await readExactly(file, Buffer.from(sharedBuffer), dataStart + relativeStart);
        return {
            data: tensor.dtype === "F16"
...
//# sourceMappingURL=model2vec.js.map