...
export function workspaceManifestPath(home) {
    return join(home, WORKSPACE_MANIFEST_FILE);
}
export function readWorkspaceManifest(home) {
    const path = workspaceManifestPath(home);
    const value = readJsonFileSync(path, null);
    if (value === null) {
        return null;
    }
...
export function writeWorkspaceManifest(home, manifest) {
...
export function deleteWorkspaceManifest(home) {
    rmSync(workspaceManifestPath(home), { force: true });
}
export function workspaceIndexInfoFromManifest(manifest) {
    return {
        id: manifest.id,
        name: manifest.name,
        path: manifest.path,
...
function isEmbeddingRuntime(value) {
    if (!isRecord(value)) {
        return false;
    }
