import { closestAncestor, extractCommonModifiers, extractGenericSignature, extractPrecedingDoc, } from "./metadata.js";
...
export function shouldIndexJavascriptTypescriptEntity(node) {
...
export function hasJavascriptTypescriptFunctionValue(node) {
    return hasFunctionValue(node);
}
export function resolveJavascriptTypescriptEntities(node) {
    if (node.type !== "variable_declarator") {
        return [node];
    }
...
export function extractJavascriptTypescriptName(node) {
    if (node.type === "pair") {
        return node.childForFieldName("key")?.text.replace(/^['"`]|['"`]$/g, "");
    }
    return (node.childForFieldName("name")?.text ?? findNamedIdentifierChild(node)?.text);
}
export function javascriptTypescriptScopeBreadcrumb(node, breadcrumb) {
    const objectName = exportedObjectVariableName(node);
    return objectName ? [...breadcrumb, objectName] : breadcrumb;
}
export function extractJavascriptTypescriptSignature(node) {
    if (node.type === "pair") {
...
export function classifyJavascriptTypescriptNode(node) {
    if (node.type === "pair" ||
...
function hasFunctionValue(node) {
    const value = node.childForFieldName("value") ??
...
function containsFunctionValue(node) {
...
function exportedObjectFunctionEntities(node) {
    ...
    return value.namedChildren.filter((child) => (child.type === "pair" && hasFunctionValue(child)) ||
        child.type === "method_definition");
}
function exportedObjectVariableName(node) {
    const object = closestAncestor(node, "object") ??
