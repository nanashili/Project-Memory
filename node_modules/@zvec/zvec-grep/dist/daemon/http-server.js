...
import { createRemoteEmbeddingRequestStateCodec, InMemoryRemoteEmbeddingRequestStateReplayGuard, requestPrincipal, } from "../mcp/request-state.js";
import { DEFAULT_MCP_TOOLSET } from "../mcp/toolset.js";
import { runWithTraceContext, traceContextFromMcpBody, } from "../observability/trace-context.js";
...
export class DaemonHttpServer {
    ...
    constructor(options) {
        ...
        const requestStateCodec = createRemoteEmbeddingRequestStateCodec(options.requestStateKey ?? randomBytes(32));
        const requestStateReplayGuard = options.requestStateReplayGuard ??
            new InMemoryRemoteEmbeddingRequestStateReplayGuard();
        this.mcpEndpoint = new McpHttpEndpoint(options.backend, options.version, {
        ...
        this.adminMcpEndpoint = new McpHttpEndpoint(options.backend, options.version, {
            toolset: "full",
    ...
    async handleRequest(request, response, id) {
        ...
        const mcpEndpoint = url.pathname === "/mcp"
            ? this.mcpEndpoint
        ...
        if (traceContext)
            this.requestTraceIds.set(id, traceContext.traceId);
...
function attachRequestPrincipal(request, token) {
    request.auth = requestPrincipal(token);
}
function validHost(hostHeader) {
    if (!hostHeader) {
        return false;
    }
    try {
...
function validOrigin(originHeader) {
    if (!originHeader) {
        return true;
    }
    try {
...
function validRequestToken(header, expected) {
