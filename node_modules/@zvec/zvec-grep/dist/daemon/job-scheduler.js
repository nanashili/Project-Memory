...
export class JobScheduler {
    jobs = new Map();
    activeByRoot = new Map();
    latestByRoot = new Map();
    queue = [];
    concurrency;
    maxAttempts;
    retryBaseDelayMs;
    ...
    async waitForRootIdle(canonicalRoot) {
        while (true) {
    ...
    async close() {
        ...
        for (const job of this.jobs.values()) {
            this.cancelJob(job, "Indexing was cancelled because the daemon is shutting down.");
        }
        this.queue.length = 0;
        if (this.running === 0) {
            return;
    ...
    sortQueue() {
        this.queue.sort((left, right) => priority(right.reason) - priority(left.reason) ||
...
function mergeQueuedJob(current, incoming) {
...
function combineRuns(first, second) {
