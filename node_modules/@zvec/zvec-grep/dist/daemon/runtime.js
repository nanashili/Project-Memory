...
import { MCP_TOOLSET_ENV, resolveMcpToolset, } from "../mcp/toolset.js";
import { loadOrCreateMcpRequestStateKey, PersistentRemoteEmbeddingRequestStateReplayGuard, } from "../mcp/request-state.js";
export async function runDaemonForeground(options) {
    const mcpToolset = resolveMcpToolset(options.mcpToolset, process.env[MCP_TOOLSET_ENV]);
    const listen = configuredListenAddress(options.listen);
    const displayAddress = `http://${displayHost(listen.host)}:${listen.port}/mcp`;
    const instanceLock = await DaemonInstanceLock.acquire(options.home, displayAddress, mcpToolset);
    ...
    const httpServer = new DaemonHttpServer({
        ...
        backend,
        requestStateKey,
        requestStateReplayGuard: new PersistentRemoteEmbeddingRequestStateReplayGuard(options.home),
        logger,
