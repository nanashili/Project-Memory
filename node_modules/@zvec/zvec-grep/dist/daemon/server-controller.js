...
import { configuredListenAddress, daemonHome, resolveClientToken, } from "./config.js";
import { processIsAlive } from "../engine/utils/daemon-lease.js";
import { DEFAULT_MCP_TOOLSET, MCP_TOOLSET_ENV, resolveMcpToolset, } from "../mcp/toolset.js";
...
export class DaemonInstanceLock {
    ...
    static async acquire(home, serverUrl, mcpToolset = DEFAULT_MCP_TOOLSET) {
    ...
    async release() {
        ...
        if (current?.instanceToken === this.record.instanceToken &&
...
export async function readInstanceRecord(home) {
    return readRecordPath(join(daemonHome(home), "instance.lock"));
}
export async function serverStatus(home) {
...
export async function startServer(options) {
    ...
    const requestedToolset = resolveMcpToolset(options.mcpToolset, environmentToolset);
    const toolsetWasExplicit = options.mcpToolset !== undefined || environmentToolset !== undefined;
    const current = await serverStatus(options.home);
    if (current.running) {
        if (toolsetWasExplicit &&
            current.mcpToolset !== undefined &&
            current.mcpToolset !== requestedToolset) {
            throw new Error(`zvec-grep server is already running with MCP toolset "${current.mcpToolset}". Run \`zg server off\`, then restart it with \`zg server on --mcp-toolset ${requestedToolset}\`.`);
        }
        if (current.ready)
            return current;
...
export async function stopServer(home, timeoutMs = 30_000, tokenFile) {
    const record = await readInstanceRecord(home);
    if (!record ||
        record.hostname !== hostname() ||
        !processIsAlive(record.pid)) {
    ...
    try {
        const response = await fetch(new URL("/control/shutdown", record.serverUrl), {
            method: "POST",
            headers: token ? { Authorization: `Bearer ${token}` } : {},
            signal: AbortSignal.timeout(Math.min(SHUTDOWN_REQUEST_TIMEOUT_MS, Math.max(timeoutMs, 100))),
        });
        if (!response.ok) {
        ...
        if (error instanceof ShutdownResponseError)
            throw error;
    }
    if (shutdownAccepted &&
...
async function forceStopRecordedProcess(home, record, allowMissingRecord, timeoutMs) {
    ...
    if (!(await waitForProcessExit(record.pid, graceMs))) {
        throw new Error(`Timed out waiting for zvec-grep server process ${record.pid} to stop after SIGKILL.`);
    }
    await removeStoppedInstanceRecord(home, record);
    return { running: false, ready: false };
}
async function assertSameInstanceBeforeSignal(home, expected, allowMissing) {
    ...
    if (!current ||
        ...
        throw new Error("zvec-grep server instance changed while stopping; refusing to signal the recorded process.");
    }
}
...
async function removeStoppedInstanceRecord(home, stopped) {
    const path = join(daemonHome(home), "instance.lock");
...
async function assertListenAddressAvailable(host, port) {
    ...
    try {
        ...
        if (error.code === "EADDRINUSE") {
            throw new Error(`Server address ${host}:${port} is already in use. Another or legacy zvec-grep server may still be running.`, { cause: error });
        }
        throw error;
    }
}
...
async function waitForStatus(home, running, timeoutMs) {
    const deadline = Date.now() + timeoutMs;
    let last = { running: false, ready: false };
    while (Date.now() < deadline) {
        last = await serverStatus(home);
        if (running ? last.running && last.ready : !last.running)
            return last;
        await new Promise((resolve) => setTimeout(resolve, 50));
    }
    throw new Error(`Timed out waiting for zvec-grep server to ${running ? "start" : "stop"}.`);
}
async function waitForRunningRecord(home, timeoutMs) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        if ((await serverStatus(home)).running)
            return true;
        await new Promise((resolve) => setTimeout(resolve, 25));
    }
    return false;
}
async function readRecordPath(path) {
    try {
