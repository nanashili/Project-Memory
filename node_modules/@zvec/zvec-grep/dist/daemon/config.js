...
export function daemonHome(home) {
    return join(home ?? defaultHome(), "daemon");
}
export function daemonTokenPath(home) {
    return join(daemonHome(home), "token");
}
export function configuredListenAddress(listen) {
    if (listen)
        return parseListenAddress(listen);
...
export function configuredServerUrl() {
    ...
    if (config.client?.serverUrl)
        return config.client.serverUrl;
