...
export function parseArgs(args) {
    ...
    for (let index = startIndex; index < args.length; index++) {
        ...
        if (arg === "--help" || arg === "-h") {
            ...
            options.installMcpTransport = parseMcpInstallTransport(readOptionValue(args, ++index, arg));
        }
        else if (arg === "--yes") {
            options.yes = true;
        }
            ...
            options.authorizationCapability = parseAuthorizationCapability(readOptionValue(args, ++index, arg));
        }
        else if (isLongOptionWithValue(arg, "--scope")) {
            options.authorizationScope = parseAuthorizationScope(valueFromLongOption(arg));
        }
        else if (arg === "--scope") {
            options.authorizationScope = parseAuthorizationScope(readOptionValue(args, ++index, arg));
        }
        else if (arg === "--rg") {
            options.rg = true;
        }
            ...
            options.refresh = parseQueryRefreshMode(readOptionValue(args, ++index, arg));
        }
            ...
            options.hybridQueries = appendQuery(options.hybridQueries, readOptionValue(args, ++index, arg), arg);
        }
        else if (isLongOptionWithValue(arg, "--fts")) {
            options.routes = appendRoute(options.routes, "fts", valueFromLongOption(arg), "--fts");
        }
        else if (arg === "--fts") {
            options.routes = appendRoute(options.routes, "fts", readOptionValue(args, ++index, arg), arg);
        }
        else if (isLongOptionWithValue(arg, "--vector")) {
            options.routes = appendRoute(options.routes, "vector", valueFromLongOption(arg), "--vector");
        }
        else if (arg === "--vector") {
            options.routes = appendRoute(options.routes, "vector", readOptionValue(args, ++index, arg), arg);
        }
        else if (arg === "--fuse") {
            options.fuse = true;
        }
            ...
            options.globs = appendValue(options.globs, readOptionValue(args, ++index, arg), arg);
        }
            ...
            options.insensitiveGlobs = appendValue(options.insensitiveGlobs, readOptionValue(args, ++index, arg), arg);
        }
        else if (isLongOptionWithValue(arg, "--type")) {
            options.fileTypes = appendValue(options.fileTypes, valueFromLongOption(arg), "--type");
        }
        else if (arg === "--type" || arg === "-t") {
            options.fileTypes = appendValue(options.fileTypes, readOptionValue(args, ++index, arg), arg);
        }
            ...
            options.excludedFileTypes = appendValue(options.excludedFileTypes, readOptionValue(args, ++index, arg), arg);
        }
        else if (arg === "--hidden") {
            options.hidden = true;
        }
            ...
            options.ignoreFiles = appendValue(options.ignoreFiles, readOptionValue(args, ++index, arg), arg);
        }
            ...
            options.maxDepth = parseNonNegativeInteger(readOptionValue(args, ++index, arg), arg);
        }
            ...
            options.maxFileSizeBytes = parseByteSize(readOptionValue(args, ++index, arg), arg);
        }
            ...
            options.rgOptions = appendRgPatternFile(options.rgOptions, readOptionValue(args, ++index, arg), arg);
            markRgCompatibilityOption(options, arg);
        }
...
function parseCommand(args) {
    ...
    if (first === "version" || first === "-v" || first === "--version") {
        const acceptsVersionFlag = first === "version" &&
            rest.length === 1 &&
...
function validateCliShape(command, options, positionals) {
    ...
    if (options.allowRemote && command !== "query" && command !== "index") {
        throw new Error("--allow-remote can only be used with query or index commands");
    }
    ...
    if (sharedSelection && command !== "query" && command !== "index") {
        throw new Error(`${sharedSelection} can only be used with query or index commands`);
    }
    ...
    if (discoveryOption &&
        ...
        throw new Error(`${discoveryOption} can only be used with index commands or zg query --rg`);
    }
    if (command === "config") {
    ...
    if (options.installTargets?.length &&
        command !== "install" &&
        command !== "uninstall") {
        throw new Error("--target can only be used with zg install or zg uninstall");
    }
    ...
    if (command === "query" && !options.rg) {
        ...
        if (queryCount === 0) {
            throw new Error("zg query requires a query or --hybrid/--fts/--vector route");
        }
    }
}
