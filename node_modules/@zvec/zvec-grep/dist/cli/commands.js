...
import { createZvecGrep, } from "../index.js";
...
import { resolveDirectSearchPolicy, resolveServerSearchPolicy, } from "../client/search-policy.js";
...
import { contextWarningLines, printCliContextResult, } from "./format/context.js";
import { printDebug } from "./format/debug.js";
import { createIndexProgressReporter } from "./format/progress.js";
import { printWorkspaceInfo, printIndexPathFilterTip, printIndexResult, printIndexScanDiagnostics, printNoIndexableFilesTip, printServerIndexInfo, } from "./format/status.js";
import { planRemoteIndexAuthorization, planRemoteSearchAuthorization, } from "../authorization/index.js";
import { indexCompletionFromStatus, indexStatusNeedsRefresh, } from "../engine/index-status.js";
import { indexProgressFromMessage } from "../index-progress.js";
import { normalizeManagedRgInput } from "./managed-rg.js";
import { INCOMPATIBLE_SERVER_SEARCH_MESSAGE, parseServerSearchResponse, } from "./server-search.js";
import { runInstall, runUninstall } from "./install.js";
import { runAuth, authorizationStore, authorizeCliPlan, resolveAuthorizationSchema, embeddingModelInfo, configuredEmbeddingReference, workspaceRuntimeFromInfo, assertEmbeddingModelCompatible, assertRequestedEndpointCompatible, requireEmbeddingModelCatalogEntry, unsupportedRemoteEmbeddingProvider, withRemoteEmbeddingOperationPermit, } from "./auth.js";
export async function runParsedCommand(parsed) {
...
async function runConfig(parsed) {
    if (parsed.positionals.length !== 1) {
        throw new Error(`zg config ${parsed.options.configAction === "provider-set" ? "provider" : "model"} set requires exactly one reference`);
    }
    const reference = parsed.positionals[0];
    if (parsed.options.configAction === "provider-set") {
        if (!/^[a-z][a-z0-9_-]*$/.test(reference)) {
            throw unsupportedRemoteEmbeddingProvider(reference, "Invalid embedding provider");
        }
        if (reference === "local" ||
    ...
    if (parsed.options.endpoint === undefined &&
        ...
        throw new Error("zg config model set requires --endpoint, --device, or --default");
    }
    if (catalogEntry.provider === "local" &&
...
async function runIndex(parsed) {
    ...
    await routeByMode({
        ...
        server: async () => {
            ...
            let result;
            try {
                result = await client.callTool("zvec_grep_index", {
            ...
            if (parsed.options.debug) {
                printIndexScanDiagnostics(result.scan_diagnostics);
            }
...
function serverIndexFailureMessage(result) {
...
async function runDirectIndex(parsed, rootPath, explicitRoot) {
    const serviceOptions = createServiceOptions(parsed.options, rootPath.absolutePath);
    ...
    try {
        ...
        const schema = resolveAuthorizationSchema(configuredEmbeddingReference(parsed.options, infoBefore.workspaceIndex?.embedding), infoBefore.workspaceIndex?.embedding);
        assertEmbeddingModelCompatible(infoBefore.workspaceIndex?.embedding, schema, parsed.options.rebuild === true);
        const workspaceRuntime = workspaceRuntimeFromInfo(infoBefore);
        assertRequestedEndpointCompatible(infoBefore, workspaceRuntime, parsed.options.endpoint, parsed.options.rebuild === true);
        const modelInfo = schema?.provider === "qwen"
        ...
        const result = await withRemoteEmbeddingOperationPermit(authorizationResolution.authorization, () => zvecGrep.index({
            root: rootPath.absolutePath,
        ...
        printIndexResult("Workspace index", result, parsed.options, info.workspaceIndex?.rootPaths);
    }
    catch (error) {
        progress.finish();
        throw error;
...
async function runDropIndex(parsed, root) {
    if (!(await confirmIndexDrop(root, parsed.options.yes === true))) {
        console.log("Index drop cancelled.");
        return;
    }
    const printResult = (removed) => {
        console.log(removed ? `Dropped index for ${root}` : `No index found for ${root}`);
    };
    await routeByMode({
        mode: resolveClientMode(parsed.options.mode),
        serverAvailable: () => daemonIsReady(parsed.options.home),
        server: async () => {
            const result = await daemonClient(parsed.options).callTool("zvec_grep_index_drop", { root });
...
async function confirmIndexDrop(root, accepted) {
    if (accepted) {
        return true;
    }
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
        throw new Error("zg index --drop requires --yes in a non-interactive shell");
    }
    ...
    try {
        const answer = await readline.question(`Drop the index for ${root}? [y/N] `);
...
async function runServer(parsed) {
    if (parsed.positionals.length > 0) {
        throw new Error(`zg server ${parsed.options.serverStdio ? "--stdio" : parsed.options.serverAction} does not accept positional arguments`);
    }
...
async function runStatus(parsed) {
    ...
    const state = await routeByMode({
        ...
        server: async () => {
            ...
            return printServerIndexInfo(result, parsed.options);
        },
        direct: async () => {
            const zvecGrep = await createZvecGrep(createServiceOptions(parsed.options, root));
            try {
                return printWorkspaceInfo(await zvecGrep.info({ root }), parsed.options);
            }
            finally {
                await zvecGrep.close();
            }
...
async function runDirectRgQuery(commandOptions, queries) {
    const zvecGrep = await createZvecGrep(createServiceOptions(commandOptions, undefined));
    try {
        const result = await zvecGrep.context(contextOptions(commandOptions, queries));
...
async function runDirectQuery(commandOptions, queries) {
    if (commandOptions.refresh === "background") {
        console.error("warning: --refresh background requires Server mode; Direct mode uses --refresh off");
    }
    ...
    try {
        const contextRequest = contextOptions(commandOptions, queries, (progressEvent) => {
        ...
        const modelInfo = !commandOptions.rg && schema?.provider === "qwen"
            ? await embeddingModelInfo(schema, createServiceOptions(commandOptions, info.root), workspaceRuntime)
            : undefined;
        const plan = modelInfo
            ? await planRemoteSearchAuthorization({
                info,
                model: modelInfo,
                search: normalizedDirectSearchInput(commandOptions, queries, info.root),
        ...
        const authorizationResolution = plan
            ...
            : {};
        const effectiveContextRequest = authorizationResolution.alternative === "local_search"
            ? ftsFallbackContextRequest(contextRequest)
            : contextRequest;
        const result = await withRemoteEmbeddingOperationPermit(authorizationResolution.authorization, () => zvecGrep.context(effectiveContextRequest));
        progress.finish();
        printCliContextResult(result, commandOptions);
        for (const line of contextWarningLines(result)) {
            console.error(line);
...
async function runServerQuery(options, queries, routes) {
    const searchPolicy = resolveServerSearchPolicy(options);
    const structuredContent = await daemonClient(options).callTool("zvec_grep_search", {
...
function printStaleIndexStatus(state, completion) {
    console.error("status: possibly_stale");
    if (!state)
        return;
    const progress = completion?.completed === undefined || completion.total === undefined
        ? ""
        : ` (${completion.completed}/${completion.total})`;
    console.error("results: served_from_current_index");
    console.error(`background_refresh: ${state}${progress}`);
}
function daemonClient(options) {
...
function ftsFallbackContextRequest(request) {
    const queries = [
        ...(request.queries ?? []),
...
async function directQueryInfo(service) {
    return await service.info({ root: process.cwd() });
}
function normalizedDirectSearchInput(options, queries, root) {
...
async function daemonIsReady(home) {
    return (await serverStatus(home)).ready;
}
function printServerControlStatus(status) {
    console.log(`Server: ${status.running ? (status.ready ? "ready" : "starting") : "stopped"}`);
    if (status.pid)
        console.log(`PID: ${status.pid}`);
    if (status.serverUrl)
        console.log(`URL: ${status.serverUrl}`);
    if (status.mcpToolset)
        console.log(`MCP toolset: ${status.mcpToolset}`);
}
function contextOptions(options, queries, onAutoUpdateProgress) {
    const policy = resolveDirectSearchPolicy(options);
    return {
...
export function createServiceOptions(options, root) {
    return {
        root,
...
function resolveIndexRoot(root) {
    if (root !== undefined) {
        return root;
    }
    return findNearestWorkspace(process.cwd())?.root ?? process.cwd();
}
function indexRootPath(path, options) {
    return {
