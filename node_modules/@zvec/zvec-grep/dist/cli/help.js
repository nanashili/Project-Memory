import { COMPONENT_CODE_FORMATS, STRUCTURED_CODE_FORMATS, } from "../engine/code-formats.js";
import { resolveMaxFileSizeBytes } from "../engine/file-size-policy.js";
import { listKnownBinaryExtensionGroups, listRecognizedFileTypes, } from "../engine/file-type.js";
import { listEmbeddingModels } from "../engine/models/index.js";
export function printHelp(version, topic) {
    if (!topic) {
...
const MAIN_ENVIRONMENT_VARIABLES = [
    "ZVEC_GREP_HOME",
    "ZVEC_GREP_MODE",
    "ZVEC_GREP_EMBEDDING",
    "ZVEC_GREP_API_KEY",
...
function fileTypesHelp() {
    const types = listRecognizedFileTypes();
    const structuredFormats = new Set(STRUCTURED_CODE_FORMATS);
    const componentFormats = new Set(COMPONENT_CODE_FORMATS);
    const codeTypes = types.filter((type) => type.kind === "code");
    const structuredCode = codeTypes.filter((type) => structuredFormats.has(type.format));
    const componentCode = codeTypes.filter((type) => componentFormats.has(type.format));
    const plainCode = codeTypes.filter((type) => !structuredFormats.has(type.format) && !componentFormats.has(type.format));
    const documentsAndData = types.filter((type) => type.kind === "text" || type.kind === "data");
...
function formatEmbeddingModels(models) {
    const rows = models.map((model) => {
        const runtime = model.provider === "local" ? "local" : "remote";
        const input = "kind" in model && model.kind === "multimodal" ? "text,image" : "text";
        const inputLimit = "maxInputTokens" in model ? model.maxInputTokens : model.contextSize;
        return [
            model.reference,
            runtime,
            input,
    ...
    return formatTable(["MODEL", "RUNTIME", "INPUT", "DIMS", "TOKENS", "BACKEND"], rows, new Set([3, 4]));
}
function formatFileTypeTable(types) {
    const rows = [...types]
...
function formatEnvironmentVariables(names) {
