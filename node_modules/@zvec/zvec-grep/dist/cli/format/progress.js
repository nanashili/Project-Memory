import { formatIndexProgressLine, formatModelDownloadProgress, } from "../../index-progress.js";
...
export function createIndexProgressReporter(options = {}) {
    ...
    let heartbeat = null;
    const writeLine = (line, force = false) => {
    ...
    const writePersistentLine = (line) => {
        if (!process.stderr.isTTY) {
            writeNonTtyProgressLine(line);
        ...
        report(progress) {
            ...
            if (modelStage === "ready") {
                ...
                if (lastIndexingProgress) {
                    const line = process.stderr.isTTY
                        ? formatTtyProgressLine(lastIndexingProgress, lastIndexingProgress, color)
            ...
            const line = process.stderr.isTTY
                ...
                : formatIndexProgressLine(progress);
            if (line)
                writeLine(line, progress.phase === "done");
        },
...
function writeTtyProgressLine(line) {
    process.stderr.write(`${ANSI_CLEAR_LINE}${line}`);
}
function writeNonTtyProgressLine(line) {
    process.stderr.write(`${line}\n`);
}
function formatTtyProgressLine(progress, lastIndexingProgress, color) {
    ...
    if (modelDownload) {
        ...
        const glyph = glyphs.spinner[downloadedBytes % glyphs.spinner.length] ??
            glyphs.spinner[0];
        return `${dim(glyphs.rail, color)}  ${green(glyph, color)} ${modelDownload}`;
    }
    const effectiveProgress = progress.phase === "done" && lastIndexingProgress
        ? {
            ...lastIndexingProgress,
            filesIndexed: lastIndexingProgress.filesTotal ??
    ...
    if (total <= 0) {
        const label = progress.phase === "done"
            ...
            : "Indexing files";
        const glyph = progress.phase === "done"
            ? glyphs.done
            : glyphs.spinner[(effectiveProgress.filesIndexed ?? 0) % glyphs.spinner.length];
        return `${dim(glyphs.rail, color)}  ${green(glyph, color)} ${label}`;
    }
    const indexed = Math.min(Math.max(effectiveProgress.filesIndexed ?? 0, 0), total);
    ...
    const barWidth = Math.max(MIN_PROGRESS_BAR_WIDTH, Math.min(MAX_PROGRESS_BAR_WIDTH, available));
    const filled = Math.round((barWidth * percent) / 100);
    return `${prefix}${gradientBar(filled, barWidth, glyphs, color)}${suffix}`;
}
function gradientBar(filled, width, glyphs, color) {
...
export function formatGreenProgressBar(completed, total, width, color) {
    const boundedTotal = Math.max(total, 0);
    const boundedWidth = Math.max(Math.floor(width), 0);
    const ratio = boundedTotal === 0
        ? 0
...
function formatTtyMetadata(progress) {
    const parts = [];
    ...
    if (retries > 0)
        parts.push(`${formatCount(retries)} retries`);
...
function shouldUseProgressColor(mode) {
    if (mode === "always")
        return true;
    if (mode === "never")
        return false;
