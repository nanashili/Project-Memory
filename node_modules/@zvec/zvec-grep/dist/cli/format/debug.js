export function printDebug(result, options = {}) {
    const items = cliVisibleItems(result);
    console.error(`source=${result.source} coverage=${result.coverage} items=${items.length}`);
    if (result.workspaceIndex) {
        console.error(`workspace_index=${result.workspaceIndex.name} path=${result.workspaceIndex.path}`);
    }
    ...
    if (result.diagnostics.rg) {
        ...
        console.error(`rg=${rg.backend} limit=${rg.limit ?? "all"} truncated=${rg.truncated}`);
        console.error(`rg_command=${shellArg(rg.command)} ${rg.args.map(shellArg).join(" ")}`);
    ...
    if (result.diagnostics.structure) {
        ...
        console.error(`structural_enrichment=${status} source=${structure.source} matched_files=${structure.matchedFiles} ` +
...
function printTrace(result, items) {
...
function cliVisibleItems(result) {
