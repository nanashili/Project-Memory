...
async function updateJsoncMcpSettings(options) {
    ...
    source = editJsonWithComments(source, ["mcpServers", "zvec_grep"], options.server);
    await writeTextFileAtomic(options.path, ensureTrailingNewline(source));
    return root;
}
async function removeJsoncMcpSettings(path, label, isManaged) {
    const existing = await readTextFileIfExists(path);
    if (!existing.trim())
        return;
    let source = existing;
...
function parseJsoncSettings(path, source, label) {
    const errors = [];
