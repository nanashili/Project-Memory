...
export async function planRemoteIndexAuthorization(input) {
    if (input.model.provider !== "qwen")
        return undefined;
    const needsEmbedding = input.rebuild === true ||
    ...
    if (endpoint === undefined) {
        throw new Error(`Embedding model ${input.model.reference} did not provide a remote endpoint.`);
    }
    ...
    return {
        ...
        disclosure: {
            queryText: false,
            workspaceContent: !input.info.indexed || input.rebuild ? "full" : "changed",
        },
...
export async function planRemoteSearchAuthorization(input) {
    ...
    if (input.model.provider !== schema.provider ||
        input.model.name !== schema.model) {
        throw new Error(`Embedding model ${input.model.reference} does not match indexed model ${schema.provider}/${schema.model}.`);
    }
    const usesVector = searchUsesVector(input.search);
    const needsUpdate = input.runtimeNeedsReconciliation === true ||
        !indexStatusIsFresh(input.info);
    const updatesIndex = needsUpdate &&
        (input.search.autoUpdate || input.search.freshness === "wait_for_fresh");
    if (!usesVector && !updatesIndex)
        return undefined;
    const endpoint = input.model.endpoint;
    if (endpoint === undefined) {
        throw new Error(`Embedding model ${input.model.reference} did not provide a remote endpoint.`);
...
export function searchUsesVector(input) {
    return ((input.queries?.length ?? 0) > 0 ||
        input.routes.some((route) => route.mode === "vector"));
}
export function indexStatusIsFresh(info) {
...
function workspaceRoots(info) {
    const roots = info.workspaceIndex?.rootPaths.map((root) => root.absolutePath) ?? [];
