...
import { acceptedContent, inputRequired, inputResponse, McpServer, ProtocolError, } from "@modelcontextprotocol/server";
import { z } from "zod";
import { formatAgentContextResult } from "../cli/format/context.js";
import { formatRemoteEmbeddingAuthorizationPrompt, remoteEmbeddingDisclosureData, } from "../authorization/prompt.js";
import { normalizeSearchInput, } from "./input-normalization.js";
import { zvecGrepIndexInputSchema, zvecGrepIndexOutputSchema, zvecGrepIndexDropInputSchema, zvecGrepIndexDropOutputSchema, zvecGrepIndexStatusInputSchema, zvecGrepIndexStatusOutputSchema, zvecGrepRgInputSchema, zvecGrepCliSearchInputSchema, zvecGrepSearchInputSchema, zvecGrepSearchOutputSchema, zvecGrepServerStatusInputSchema, zvecGrepServerStatusOutputSchema, } from "./schemas.js";
...
import { DEFAULT_MCP_TOOLSET } from "./toolset.js";
import { createRemoteEmbeddingRequestStateCodec, InMemoryRemoteEmbeddingRequestStateReplayGuard, matchesRemoteEmbeddingRequestState, remoteEmbeddingRequestState, } from "./request-state.js";
import { runWithTraceContext, traceContextFromMcpMeta, } from "../observability/trace-context.js";
import { formatPromptRules, ZVEC_GREP_WORKSPACE_EVIDENCE_RULES, } from "../prompts/zvec-grep-guidance.js";
function searchRoutingRules(exactTool, focusedTools) {
    return [
...
const ZVEC_GREP_AGENT_SEARCH_MCP_INSTRUCTIONS = searchRoutingRules("native Grep or rg", "Read, Grep, or rg");
const ZVEC_GREP_FULL_SEARCH_MCP_INSTRUCTIONS = searchRoutingRules("zvec_grep_rg", "Read or zvec_grep_rg");
...
export const ZVEC_GREP_AGENT_MCP_INSTRUCTIONS = formatPromptRules("Use zvec-grep with these workspace retrieval rules:", [
...
export const ZVEC_GREP_FULL_MCP_INSTRUCTIONS = formatPromptRules("Use zvec-grep with these workspace retrieval and lifecycle rules:", [
    ...ZVEC_GREP_WORKSPACE_EVIDENCE_RULES,
...
export function createZvecGrepMcpServer(backend, version, options = {}) {
    ...
    const requestStateReplayGuard = options.requestStateReplayGuard ??
        new InMemoryRemoteEmbeddingRequestStateReplayGuard();
    const server = new McpServer({ name: "zvec-grep", version }, {
...
export function registerZvecGrepTools(server, backend, options = {}) {
    const toolset = options.toolset ?? DEFAULT_MCP_TOOLSET;
    const full = toolset === "full";
    if (full) {
        server.registerTool("zvec_grep_index", {
        ...
        }, async (input, ctx) => await runWithTraceContext(traceContextFromMcpMeta(ctx.mcpReq._meta), async () => {
            const request = {
                ...input,
            ...
            const resolution = plan
                ? await resolveRemoteEmbeddingAuthorization(backend, plan, undefined, ctx, "zvec_grep_index", request, options)
                : { kind: "ready" };
            if (resolution.kind === "input_required")
    ...
    server.registerTool("zvec_grep_search", {
        ...
        const resolution = plan
            ? await resolveRemoteEmbeddingAuthorization(backend, plan, "local_search", ctx, "zvec_grep_search", normalized, options)
            : { kind: "ready" };
        if (resolution.kind === "input_required")
            return resolution.result;
        const effectiveSearch = resolution.alternative === "local_search"
            ...
            ? toolResult(text, {
                ...
                result: {
                    ...
                    groupResults: response.result.groupResults?.map((group) => ({
                        ...group,
                        items: group.items.map(({ queryGroups: _queryGroups, selectionReason: _selectionReason, coverageGroup: _coverageGroup, ...item }) => item),
                    })),
                },
            })
            : textToolResult(text);
    ...
    if (full) {
        server.registerTool("zvec_grep_index_drop", {
    ...
    if (full) {
        server.registerTool("zvec_grep_rg", {
            ...
            return textToolResult(`${formatAgentContextResult(response.result, {})}${rgCoverageHint(response.result)}`);
        });
    }
    if (full) {
        server.registerTool("zvec_grep_index_status", {
            title: "Inspect zvec-grep index status",
            description: "Read persisted index status and, when active, daemon runtime and job status for an absolute root. Use only after a missing-index response, indexing failure or cancellation, explicit progress monitoring, or daemon diagnostics.",
        ...
        server.registerTool("zvec_grep_server_status", {
...
function rgCoverageHint(result) {
...
function createMcpIndexProgressReporter(extra) {
    ...
    return {
        report(progress) {
            ...
            else if (modelPreparationActive &&
...
const REMOTE_EMBEDDING_SCOPE_DESCRIPTION = "Allow this operation once or remember permission for this workspace.";
...
async function resolveRemoteEmbeddingAuthorization(backend, plan, alternative, ctx, tool, args, options = {}) {
    const existing = await backend.existingRemoteEmbeddingPermit?.(plan);
    if (existing)
        return { kind: "ready", authorization: existing };
    ...
    const echoedState = ctx.mcpReq.requestState();
    if (echoedState) {
        ...
        const response = inputResponse(ctx.mcpReq.inputResponses, "remote_embedding_authorization");
        const content = acceptedContent(ctx.mcpReq.inputResponses, "remote_embedding_authorization", remoteEmbeddingDecisionSchema);
        if (response.kind !== "elicit" ||
            response.action !== "accept" ||
            !content) {
            throw new Error("Remote Embedding authorization was declined. No remote data was sent.");
        }
        ...
        if (content.decision !== "allow_once" &&
            content.decision !== "allow_workspace") {
            throw new Error("Remote Embedding authorization was declined. No remote data was sent.");
        }
        const scope = content.decision === "allow_workspace" ? "workspace" : "once";
        return {
            kind: "ready",
    ...
    return {
        ...
        result: inputRequired({
            requestState: await requestStateCodec.mint(options.requestStateReplayGuard.issue(expectedState), ctx),
            inputRequests: {
...
function ftsFallbackSearch(input) {
    const queries = [
        ...(input.queries ?? []),
...
function formatSearchIndexing(indexing) {
