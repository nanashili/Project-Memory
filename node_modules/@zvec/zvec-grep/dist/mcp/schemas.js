...
const boundedString = (description) => z.string().max(MCP_MAX_QUERY_CHARS).describe(description);
const boundedStringList = (description) => z
    .union([
...
const fileTypesDescription = "Ripgrep file type names from rg --type-list to include, such as ts, py, h, or cpp.";
const excludedFileTypesDescription = "Ripgrep file type names from rg --type-list to exclude.";
...
const searchFields = {
    query: boundedString("One primary hybrid-search group using natural-language or exact terms.").optional(),
    queries: boundedStringList("One or more primary hybrid-search groups. By default, each group is searched separately and retains group metadata."),
    fts: boundedStringList("Supplemental lexical-route groups for exact anchors such as symbols, flags, or error messages; these are retrieval routes, not hard result constraints."),
    vector: boundedStringList("Supplemental semantic/vector-route groups; these are retrieval routes, not hard result constraints."),
    limit: z
        .number()
        .int()
        .positive()
        ...
        .optional()
        .describe("Maximum returned items per query group, or for the single fused plan."),
    globs: pathFilterInputSchema.describe("Ordered case-sensitive rg-style glob rules. Later rules override earlier rules."),
    insensitiveGlobs: pathFilterInputSchema.describe("Ordered case-insensitive rg-style glob rules. Later rules override earlier rules."),
    fileTypes: pathFilterInputSchema.describe(fileTypesDescription),
    excludedFileTypes: pathFilterInputSchema.describe(excludedFileTypesDescription),
    hidden: z.boolean().optional().describe("Include hidden paths."),
    noIgnore: z
        .boolean()
        .optional()
        .describe("Do not respect ignore files such as .gitignore."),
    ignoreFiles: pathFilterInputSchema.describe("Additional ignore files relative to the workspace root."),
    maxDepth: z
        .number()
        .int()
        .nonnegative()
    ...
    fuse: z
        .boolean()
        .optional()
        .describe("Collapse all primary and supplemental groups into one ranked search plan; otherwise search groups separately and retain group metadata."),
    preferSymbol: z
        .boolean()
        .optional()
...
export const zvecGrepIndexInputSchema = z.object({
    ...
    drop: z
        .boolean()
        .optional()
        .describe("Permanently remove the workspace index. Use only when index deletion is explicitly requested, and do not combine with indexing options."),
    embedding: z
        .string()
        .trim()
        .min(1)
        .max(256)
        .optional()
        .describe("Embedding model reference for a new index. Use a user-selected embedding, or omit only when a server default model is known; never guess a model."),
    rebuild: z
        .boolean()
        .optional()
        .describe("Explicitly rebuild the existing index. Use only when rebuild was requested or required by an incompatible schema or index version."),
    resetPaths: z
        .boolean()
        .optional()
        .describe("Replace the index root-path configuration."),
    globs: pathFilterInputSchema.describe("Ordered case-sensitive rg-style glob rules for indexed files."),
    insensitiveGlobs: pathFilterInputSchema.describe("Ordered case-insensitive rg-style glob rules for indexed files."),
    fileTypes: pathFilterInputSchema.describe(fileTypesDescription),
    excludedFileTypes: pathFilterInputSchema.describe(excludedFileTypesDescription),
    hidden: z.boolean().optional().describe("Include hidden paths."),
    noIgnore: z
    ...
    wait: z
        .boolean()
        .optional()
        .describe("Wait for the submitted index job to finish. Defaults to false: submit in the background and poll zvec_grep_index_status only when completion, progress monitoring, or diagnostics are required."),
});
...
const cliSearchRouteInputSchema = z.object({
    ...
    query: boundedString("One ordered supplemental retrieval group supplied by the CLI."),
});
...
export const zvecGrepCliSearchInputSchema = zvecGrepSearchInputSchema.extend({
    ...
    routes: z
        ...
        .optional()
        .describe("Ordered supplemental retrieval groups. Internal CLI transport only."),
});
...
export const zvecGrepRgInputSchema = z.object({
    root: absoluteRootSchema.describe("Absolute workspace root visible to the daemon. Keep this at the workspace root; scope the search with command paths or globs."),
    command: z
        .string()
        .trim()
        .min(1, "command is required.")
        .max(MCP_MAX_QUERY_CHARS)
        .describe("The command MUST start with `rg`; it is parsed as arguments and never executed by a shell. Search is exhaustive by default; append `| head -N` only to request a bounded result set."),
});
...
export const zvecGrepIndexOutputSchema = z.object({
    ...
    scan_diagnostics: z
        ...
        skippedSamples: z.array(z.object({
            absolutePath: z.string(),
            relativePath: z.string(),
