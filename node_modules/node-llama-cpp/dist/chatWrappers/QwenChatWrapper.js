...
export class QwenChatWrapper extends ChatWrapper {
    ...
    /** @internal */ _ensureModelThoughtBeforeTextOnLastResponse;
    settings;
    constructor(options = {}) {
        super();
        const { keepOnlyLastThought = true, thoughts = "auto", variation = "3", _lineBreakBeforeFunctionCallPrefix = false, _flatFunctionResultString = false, _ensureModelThoughtBeforeTextOnLastResponse = false } = options;
        ...
        for (let i = 0; i < historyWithFunctions.length; i++) {
            ...
            if (item.type === "system") {
                ...
                if (this.keepOnlyLastThought && !isLastItem)
                    transformedModelResponse = transformedModelResponse.filter((response) => (!isChatModelResponseSegment(response) || response.segmentType !== "thought"));
                else if (isLastItem && this._ensureModelThoughtBeforeTextOnLastResponse) {
                    transformedModelResponse = transformedModelResponse.flatMap((response, index) => {
